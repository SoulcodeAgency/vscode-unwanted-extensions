...
export async function getExtensionsJson(verbose = false): Promise<Configs> {
    ...
    if (files.length === 0) {
        verbose && vscode.window.showWarningMessage('No defined extensions found, please define "unwantedRecommendations" within the ".vscode/extensions.json" file. See [documentation](https://github.com/SoulcodeAgency/vscode-unwanted-recommendations) for more details.');
